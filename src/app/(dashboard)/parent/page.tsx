...
const ParentDashboard = async () => {
  ...
  // Handle case where parent record doesn't exist
  if (!parent) {
    return (
      <div className="flex-1 p-6">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          ...
          <p className="text-yellow-700 mt-2">
            Your parent profile is not yet set up. Please contact the school administration.
          </p>
        </div>
      </div>
    );
  ...
  const totalFees = parent?.students.reduce((sum, student) => {
    return sum + (student.feeBalances ? Number(student.feeBalances.totalFees) : 0);
  }, 0) || 0;
  
  const totalPaid = parent?.students.reduce((sum, student) => {
    return sum + (student.feeBalances ? Number(student.feeBalances.amountPaid) : 0);
  }, 0) || 0;
  ...
  return (
    <div className="flex-1 p-6 space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <div className="flex items-center gap-4">
            <div className={`rounded-full p-3 ${balance > 0 ? 'bg-orange-100' : 'bg-green-100'}`}>
              <svg className={`w-6 h-6 ${balance > 0 ? 'text-orange-600' : 'text-green-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColors[paymentStatus]}`}>
                  {paymentStatus}
                </span>
              </div>
            </div>
